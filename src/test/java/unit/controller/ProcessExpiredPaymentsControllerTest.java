 ...
 */
@ExtendWith(MockitoExtension.class) @DisplayName("ProcessExpiredPaymentsController - Testes Unitários")
class ProcessExpiredPaymentsControllerTest {
	...
	@Test @DisplayName("Deve processar pagamentos expirados e publicar eventos de expiração com sucesso")
	void shouldProcessExpiredPaymentsAndPublishEventsSuccessfully() {
		...
		try (var getExpiredMock = mockStatic(GetExpiredPaymentsUseCase.class);
			...
			getExpiredMock.when(() -> GetExpiredPaymentsUseCase.getExpiredPayments(any()))
					.thenReturn(List.of(expiredPayment));

			updateStatusMock
					.when(() -> UpdatePaymentStatusUseCase.updatePaymentStatus(expiredPayment, PaymentStatus.CANCELLED))
					.thenReturn(updatedPayment);

			when(paymentDataSource.save(any())).thenAnswer(invocation -> invocation.getArgument(0));

			// Act & Assert
			assertThatNoException().isThrownBy(() -> ProcessExpiredPaymentsController
					.processExpiredPayments(paymentDataSource, eventPublisherSource));

			// Verify
			getExpiredMock.verify(() -> GetExpiredPaymentsUseCase.getExpiredPayments(any()));
			updateStatusMock.verify(
					() -> UpdatePaymentStatusUseCase.updatePaymentStatus(expiredPayment, PaymentStatus.CANCELLED));
			publishEventMock.verify(() -> PublishPaymentExpiredEventUseCase.publishPaymentExpiredEvent(any(), any()));
		}
	}

	@Test @DisplayName("Não deve fazer nada quando não houver pagamentos expirados")
