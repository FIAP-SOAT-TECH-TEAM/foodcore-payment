...
@DisplayName("ProcessPaymentNotificationController - Testes Unitários")
class ProcessPaymentNotificationControllerTest {
	...
	@Test
	@DisplayName("Deve processar notificação de pagamento com sucesso e publicar evento quando aprovado")
	void shouldProcessNotificationAndPublishApprovedEventSuccessfully() {
		...
		try (var processMock = mockStatic(ProcessPaymentNotificationUseCase.class);
			...
			processMock.when(() ->
							ProcessPaymentNotificationUseCase.processPaymentNotification(any(), any(), any()))
					.thenReturn(payment);
			...
			assertThatNoException().isThrownBy(() ->
					ProcessPaymentNotificationController.processPaymentNotification(
							request, paymentDataSource, acquirerSource, eventPublisherSource)
			);

			processMock.verify(() ->
					ProcessPaymentNotificationUseCase.processPaymentNotification(any(), any(), any()));
			publishMock.verify(() ->
					PublishPaymentApprovedEventUseCase.publishPaymentApprovedEvent(any(), any()));
		}
	}

	@Test
	@DisplayName("Não deve publicar evento quando pagamento não for aprovado")
	...
	@Test
	@DisplayName("Deve capturar PaymentAlreadyProcessedException sem lançar exceção")
