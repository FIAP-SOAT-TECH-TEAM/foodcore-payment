...
@DisplayName("GetLatestOrderPaymentByOrderIdController - Testes Unitários")
class GetLatestOrderPaymentByOrderIdControllerTest {
	...
	@Test
	@DisplayName("Deve retornar PaymentResponse com sucesso ao obter pagamento mais recente")
	void shouldReturnPaymentResponseSuccessfully() {
		...
		try (var useCaseMock = mockStatic(GetLatestPaymentByOrderIdUseCase.class)) {
			...
			useCaseMock.when(() -> GetLatestPaymentByOrderIdUseCase
							.getLatestPaymentByOrderId(eq(orderId), any(PaymentGateway.class), isNull()))
					.thenReturn(payment);

			// Act
			var result = GetLatestOrderPaymentByOrderIdController
					.getLatestOrderPaymentByOrderId(orderId, paymentDataSource);
	...
	@Test
	@DisplayName("Deve chamar GetLatestPaymentByOrderIdUseCase com orderId correto")
	void shouldCallUseCaseWithCorrectOrderId() {
		...
		try (var useCaseMock = mockStatic(GetLatestPaymentByOrderIdUseCase.class)) {
			...
			// Act
			GetLatestOrderPaymentByOrderIdController
					.getLatestOrderPaymentByOrderId(orderId, paymentDataSource);
	...
	@Test
	@DisplayName("Deve retornar null quando PaymentPresenter retornar null")
	void shouldReturnNullWhenPresenterReturnsNull() {
		...
		try (var useCaseMock = mockStatic(GetLatestPaymentByOrderIdUseCase.class);
			...
			presenterMock.when(() -> PaymentPresenter.toPaymentResponse(payment))
					.thenReturn(null);
