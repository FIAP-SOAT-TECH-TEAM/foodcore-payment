 ...
 * eventos de pagamento aprovados e expirados.
 * </p>
 */
...
class EventPublisherGatewayTest {
	...
	@Test @DisplayName("Deve publicar evento PaymentApprovedEvent com sucesso")
	void shouldPublishPaymentApprovedEventSuccessfully() {
		...
		verify(eventPublisherSource).publishPaymentApprovedEvent(PaymentApprovedEventMapper.toDto(event));
	}

	@Test @DisplayName("Deve publicar evento PaymentExpiredEvent com sucesso")
	void shouldPublishPaymentExpiredEventSuccessfully() {
		...
		verify(eventPublisherSource).publishPaymentExpiredEvent(PaymentExpiredEventMapper.toDto(event));
	}
	...
	void shouldDelegateCorrectlyForMultipleEvents() {
		...
		PaymentExpiredEvent expiredEvent = EventFixture.createPaymentExpiredEvent(321L, java.time.LocalDateTime.now());

		gateway.publishPaymentApprovedEvent(approvedEvent);
		gateway.publishPaymentExpiredEvent(expiredEvent);

		verify(eventPublisherSource).publishPaymentApprovedEvent(PaymentApprovedEventMapper.toDto(approvedEvent));
		verify(eventPublisherSource).publishPaymentExpiredEvent(PaymentExpiredEventMapper.toDto(expiredEvent));
	}
}
