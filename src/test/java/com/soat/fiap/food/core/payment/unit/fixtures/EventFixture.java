package com.soat.fiap.food.core.payment.unit.fixtures;
...
public class EventFixture {
	 ...
	 * @return instância de PaymentApprovedEvent
	 */
	public static PaymentApprovedEvent createPaymentApprovedEvent(Long orderId, BigDecimal amount, String paymentMethod) {
		return new PaymentApprovedEvent(
				UUID.randomUUID(),
				orderId,
				amount,
	 ...
	 *
	 * @param orderId ID do pedido
	 * @param expiredIn Data de expiração do pagamento
	 * @return instância de PaymentExpiredEvent
	 */
	...
	/**
	 * Cria um StockDebitEventDto com um item padrão para testes de débito de estoque.
	 *
	 * @param orderId ID do pedido
	 * @param orderNumber Número do pedido
	 * @param userId ID do usuário (pode ser null)
	 * @param totalAmount Valor total do pedido
	 * @return instância de StockDebitEventDto
	 */
	public static StockDebitEventDto createStockDebitEvent(Long orderId, String orderNumber, String userId, BigDecimal totalAmount) {
	...
	/**
	 * Cria um StockDebitEventDto com múltiplos itens para testes de débito de estoque.
	 *
	 * @param orderId ID do pedido
	 * @param orderNumber Número do pedido
	 * @param userId ID do usuário (pode ser null)
	 ...
	 * @return instância de StockDebitEventDto
	 */
	public static StockDebitEventDto createStockDebitEventWithMultipleItems(Long orderId, String orderNumber, String userId, BigDecimal totalAmount) {
		int quantity1 = 1;
		int quantity2 = 3;
		int quantity3 = 7;
		int totalQuantity = quantity1 + quantity2 + quantity3;

		BigDecimal unitPrice1 = totalAmount.multiply(BigDecimal.valueOf(quantity1)).divide(BigDecimal.valueOf(totalQuantity), 2, RoundingMode.HALF_UP);
		BigDecimal unitPrice2 = totalAmount.multiply(BigDecimal.valueOf(quantity2)).divide(BigDecimal.valueOf(totalQuantity), 2, RoundingMode.HALF_UP);
		BigDecimal unitPrice3 = totalAmount.multiply(BigDecimal.valueOf(quantity3)).divide(BigDecimal.valueOf(totalQuantity), 2, RoundingMode.HALF_UP);
