package com.soat.fiap.food.core.payment.unit.event.handlers;
...
import com.soat.fiap.food.core.payment.unit.fixtures.EventFixture;

@ExtendWith(MockitoExtension.class)
@DisplayName("StockDebitHandler - Testes Unitários")
class StockDebitHandlerTest {
	...
	@Test
	@DisplayName("Deve processar evento de débito de estoque com sucesso")
	void shouldHandleStockDebitEventSuccessfully() {
		// Arrange
		StockDebitEventDto event = EventFixture.createStockDebitEvent(1L, "ORD-001", "user-123", BigDecimal.valueOf(100));
		var validPayment = PaymentDTOMapper.toDTO(PaymentFixture.createPendingPayment());
		when(acquirerSource.generateQrCode(any(), any()))
				.thenReturn("QR123456");
	...
	@Test
	@DisplayName("Deve processar evento de débito de estoque com múltiplos itens")
	void shouldHandleStockDebitEventWithMultipleItems() {
		// Arrange
		StockDebitEventDto event = EventFixture.createStockDebitEventWithMultipleItems(2L, "ORD-002", "user-456", BigDecimal.valueOf(150));
		var validPayment = PaymentDTOMapper.toDTO(PaymentFixture.createPendingPayment());
		when(acquirerSource.generateQrCode(any(), any()))
				.thenReturn("QR123456");
		when(paymentDataSource.save(any(PaymentDTO.class))).thenReturn(validPayment);
	...
	@Test
	@DisplayName("Deve chamar InitializePaymentController.initializePayment ao processar evento")
	void shouldCallInitializePaymentController() {
		// Arrange
		StockDebitEventDto event = EventFixture.createStockDebitEvent(3L, "ORD-003", "user-789", BigDecimal.valueOf(200));
		...
		try (MockedStatic<InitializePaymentController> mockedStatic = mockStatic(InitializePaymentController.class)) {
			...
			// Assert
			mockedStatic.verify(() -> InitializePaymentController.initializePayment(event, paymentDataSource, acquirerSource), times(1));
		}
	}

	@Test
	@DisplayName("Deve processar eventos com valores customizados")
	void shouldHandleStockDebitEventWithCustomAmount() {
		// Arrange
		StockDebitEventDto event = EventFixture.createStockDebitEvent(4L, "ORD-004", "user-999", BigDecimal.valueOf(999.99));
		var validPayment = PaymentDTOMapper.toDTO(PaymentFixture.createPendingPayment());
		when(acquirerSource.generateQrCode(any(), any()))
				.thenReturn("QR123456");
		when(paymentDataSource.save(any(PaymentDTO.class))).thenReturn(validPayment);
	...
	@Test
	@DisplayName("Deve processar evento mesmo com lista de itens vazia")
